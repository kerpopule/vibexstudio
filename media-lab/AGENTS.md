# AGENTS.md — Media Lab setup and control guide

## Goal

A fresh user can give this repository URL to a capable agent or run one documented terminal command, choose capability packs, review terms and resource costs, install verified local components, and open Media Lab with a working first-use flow.
...
2. Verify Python 3.11+, OS/architecture, GPU/driver/CUDA, RAM, disk, container runtime, ports, and writable data/model/cache roots.
3. Run `media-lab-setup` against the pinned catalog. Never bypass blocked or unqualified entries.
4. Show the user selected capabilities, dependencies, disk requirement, measured phase memory floors, runtime swap groups, source revisions/hashes, and license terms.
5. Obtain the user's direct acceptance for every gated model/runtime. An agent cannot accept third-party terms for the user.
6. Download only manifest-pinned sources; verify bytes and SHA-256 before making an artifact loadable.
...
11. Report installed and blocked capabilities, health endpoints, exact revisions, and rollback/uninstall instructions.

## External control contract

Use supported CLI/API operations rather than editing state files or killing processes:
...
- Local inference only; never fall back to a cloud model API.
- Never silently substitute an engine, variant, quantization, audio contract, or pipeline.
...
- Do not call media final before technical QA and user creative approval.
- Publication, customer data, external sharing, paid actions, public endpoints, signing, and production cutover require separate explicit approval.
...
- execute free-form generated shell commands directly.

The complete deterministic terminal/web onboarding remains available when no LLM is selected or it cannot load.

## Setup completion receipt

Require platform/hardware facts, selected/skipped capability IDs, terms identifiers, exact source revisions, bytes and SHA-256, installer exit codes, adapter-manifest matches, tracer and decode results, app health, model-swap restoration, no-secret checks, and uninstall/rollback instructions.
