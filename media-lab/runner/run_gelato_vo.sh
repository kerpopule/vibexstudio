...
for i in $(seq 1 720); do
  N=$(curl -s -m 5 "$MEDIA_LAB_STUDIO_URL/api/queue" | python3 -c 'import json,sys; print(len(json.load(sys.stdin).get("active",[])))' 2>/dev/null || echo 99)
  [ "$N" = 0 ] && break
  sleep 30
done
...
docker run --rm --name media-lab-tts-oneshot --gpus all --network none --ipc host \
  ...
  --mount "type=bind,src=$MEDIA_LAB_HOME/runner/tts_chatterbox_oneshot.py,dst=/work/tts.py,readonly" \
