...
export function SavedCollections({origin}:{origin:string}) {
  ...
  return <View style={{gap:10}}>
    ...
    {open?<>
      ...
      {/* Opening, another server, another collection or a retry mounts a fresh list: new load, cleared search and selection. */}
      <SavedCollectionList key={`${origin}\n${name}\n${attempt}`} origin={origin} name={name} onRetry={()=>setAttempt(value=>value+1)}/>
    </>:null}
  </View>;
}

function SavedCollectionList({origin,name,onRetry}:{origin:string;name:CollectionName;onRetry:()=>void}) {
  ...
  return <>
    {error?<><ThemedText accessibilityRole="alert">{error}</ThemedText><Button title="Try again" variant="secondary" onPress={onRetry}/></>:null}
